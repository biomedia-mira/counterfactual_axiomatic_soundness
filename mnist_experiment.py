...
#     if confound:
#         suffix = f'_confounded_{confound_strength:.2f}_from_joint_{from_joint}'
#     else:
#         suffix = 'not_confounded'
#
...
#
#     scenario_unconfounded = scenario_fn(data_dir, False)
#     scenario = scenario_fn(data_dir, confound, confound_strength)
...
#          LeakyRelu,
#          Conv(input_shape[-1], filter_shape=(3, 3), strides=(1, 1), padding='SAME'))
#
#     # Conditional VAE baseline
#     latent_dim = 16
#     vae_encoder = serial(parallel(serial(*encoder_layers), Pass), FanInConcat(axis=-1),
...
if __name__ == '__main__':
    ...
    #
    configs = [
        Config(baseline=True, partial_mechanisms=False, constraint_function_power=1, confound=True,
               confound_strength=.9),
        Config(baseline=False, partial_mechanisms=True, constraint_function_power=1, confound=True,
               confound_strength=.9),
        Config(baseline=False, partial_mechanisms=False, constraint_function_power=1, confound=True,
               confound_strength=.9),
    ]
    confounded_mnist(args.data_dir, 'digit_thickness_intensity', confound=True)
    # configs = [
    #     Config(baseline=False, partial_mechanisms=False, constraint_function_power=1, confound=True, de_confound=True),
    # ]
    #
    ...
    #                    confound_strength=config.confound_strength)
