...
def gradient_based_conv_transpose(lhs,
    ...
    return jax.lax.conv_general_dilated(lhs, rhs, one, pads, strides, dilation, dn, feature_group_count,
                                        precision=precision)
...
def synthesis_layer(latent_dim: int,
    ...
    def apply_fn(params: Params, inputs: Tuple[Array, Array], rng: KeyArray, **kwargs: Any) -> Tuple[Array, Array]:
        ...
        x = modulated_conv2d_layer(x=x, w=w_conv, s=style, fmaps=fmaps_out, kernel=kernel_size, up=up,
                                   demodulate=demodulate, fused_modconv=False, resample_kernel=(1, 3, 3, 1)) + b_conv
