...
def get_data(data_dir: Path, data_config_path: Path) -> Tuple[str, str, Scenario, Scenario]:
    ...
    confound_strength = float(config['confound_strength'])
    _, scenario_unconfounded = brains(data_dir=data_dir,
                                      confound=False,
                                      confound_strength=confound_strength,
                                      batch_size=512)
    dataset_name, scenario = brains(data_dir=data_dir,
                                    confound=confound,
                                    confound_strength=confound_strength,
                                    batch_size=512)
...
def main(job_dir: Path,
    ...
    parent_names = list(scenario.parent_dists.keys())
