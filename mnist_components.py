...
from model.train import Params
...
def get_components(parent_name: str, parent_dims: Dict[str, int], noise_dim: int):
    critic_layers = serial(condition_on_parents(parent_dims), *layers, Flatten, Dense(1))
    critic = f_gan(critic_layers, mode='gan', trick_g=True)
    ...
    return mechanism(parent_dims[parent_name], noise_dim), abductor, critic
