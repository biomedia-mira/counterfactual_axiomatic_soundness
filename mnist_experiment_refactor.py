...
from components.stax_extension import layer_norm, LayerNorm2D, Reshape, Array, PRNGKey, StaxLayer, Shape
from datasets.confounded_mnist import create_confounded_mnist_dataset
from model.model_refactor import build_model
...
if __name__ == '__main__':
    ...
    for parent_name, parent_dim in parent_dims.items():
        model = classifier_wrapper(parent_dim, classifier_layers)
        ...
        if model_path.exists():
            ...
            train_data = tfds.as_numpy(train_data[target_dist].batch(batch_size, drop_remainder=True).prefetch(tf.data.AUTOTUNE))
    ...
    exit(1)
    train_data = tf.data.Dataset.zip(train_data)
